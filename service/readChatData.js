class ReadChatData{
 ...
 loadIndividualData(id){
     this.readData().then(data =>{
       ...
       msgElement.addEventListener('input',()=>{
          ...
          msgElement.style.height = Math.min(msgElement.scrollHeight, 200) + "px";
           })
           inputDiv.innerHTML += '<div class="sendIcon"> <i style="color: white;" class="fa fa-paper-plane"></i></div>'
 ...
 searchMessages(input){
   this.readData().then(data => {
    var messages=[];
     data.forEach(person => {
       messages.push(...person.messages.their.concat(person.messages.yours).map((msg)=> {return  {'imageUrl': person.imageUrl,'name' : person.name, ...msg}}))
     })
